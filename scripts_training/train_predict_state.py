...
# data_dir = os.path.join(data_dir, "Cubed_400km")
...
def main(cfg_path, exp_dir, data_dir, force=False):
    ...
    data_dynamic = xr.open_zarr(
        os.path.join(data_dir, "Data", "dynamic", "time_chunked", "dynamic.zarr")
    )
    # TODO: prepare terrain features
    data_static = xr.open_zarr(os.path.join(data_dir, "Data", "static.zarr"))
    ...
    bc_scaler = LoadScaler(
        os.path.join(data_dir, "Scalers", "GlobalStandardScaler_bc.nc")
    )
    ...
    # - Compute skills
    ds_obs = xr.open_zarr(os.path.join(data_dir, "Data", "dynamic", "space_chunked", "dynamic.zarr"))
